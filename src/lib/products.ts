...
function parseCSV(csv: string): Omit<Product, 'unit'>[] {
  ...
  const SKU_HEADER_INDEX = findHeaderIndex(['sku', 'cód.']);
  const DESCRICAO_HEADER_INDEX = findHeaderIndex(['descricao', 'descrição', 'description']);
  ...
  const EAN_HEADER_INDEX = findHeaderIndex(['ean', 'barcode']);
  const EMBALAGEM_HEADER_INDEX = findHeaderIndex(['embalagem', 'packaging']);
  const CURVA_ABC_HEADER_INDEX = findHeaderIndex(['curva_abc', 'classification', 'class']);
  ...
  return lines.slice(1).map((line) => {
    ...
    const getSafeFloat = (index: number) => parseFloat(values[index]) || 0;
